...
| 날짜 | 작성자 | 브랜치/PR | 내용 |
| --- | --- | --- | --- |
| 2026-08-16 | 박소정 | `feature/chatbot` | TTS 체감 재생 속도 개선 — "음성으로 듣기" 버튼을 눌러야 그때 합성을 시작해서 7~8초씩 기다려야 했던 문제. 스트리밍 API(`generate_content_stream`)로 부분 재생이 되는지 먼저 확인해봤으나 TTS 모델이 청크를 안 쪼개고 완성된 오디오를 한 번에만 보내줘서 소용없었음 — 대신 챗봇 텍스트 답변이 도착하는 즉시 `(tabs)/chat.tsx`에서 백그라운드로 TTS를 미리 요청해 `speechCacheRef`에 캐싱해두고, "듣기"를 누르면 새로 만들지 않고 캐시를 바로 재생하도록 변경(자동재생 아님 — 소리는 여전히 버튼을 눌러야만 남). **트레이드오프**: 사용자가 안 들어도 답변마다 TTS 호출이 자동으로 나가서 무료 티어 하루 10건 한도를 대화 횟수만큼 소모함(텍스트 대화·마이크 인식 자체는 별도 모델이라 영향 없음) — 알고 결정함. 브라우저에서 실제 네트워크 요청으로 프리페치가 답변 직후 자동 발동하고 "듣기" 클릭 시 추가 요청 없이 즉시 재생되는 것 확인 |
